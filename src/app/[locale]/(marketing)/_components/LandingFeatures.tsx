...
import { type DayType, brutCard, dayCell } from './landing-shared';
    ...
    <section className='px-7 py-24' id='features'>
      <div className='max-w-[900px] mx-auto mb-14 text-center'>
        ...
        <h2 className='font-display font-extrabold leading-none tracking-[-0.03em] mb-4 text-[clamp(36px,5vw,64px)]'>
          {t('features.h2Start')}{' '}
          <em className='font-serif italic font-semibold'>{t('features.h2Em')}</em>{' '}
          {t('features.h2End')}
        </h2>
      ...
      <div className='max-w-[1240px] mx-auto grid grid-cols-1 md:grid-cols-6 gap-6'>
        {/* Big bridge feature */}
        <div className={`${brutCard} md:col-span-4 p-7 bg-[var(--accent)] transition-all duration-75 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[9px_9px_0_0_var(--frame)]`}>
          ...
          <div className='grid grid-cols-7 gap-1 max-w-[280px]'>
            {(['weekend', 'pto', 'holiday', 'work', 'work', 'work', 'work'] as DayType[]).map((dt, i) => (
              <div key={i} className={`${dayCell[dt]} aspect-square grid place-items-center text-[12px] font-bold`}>
                {['L','M','X','J','V','S','D'][i]}
              </div>
            ))}
          </div>
        ...
        {/* Ratio max */}
        <div className={`${brutCard} md:col-span-2 p-7 transition-all duration-75 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[9px_9px_0_0_var(--frame)]`}>
        ...
        >
          ...
          <h3 className='font-display font-extrabold text-[26px] mt-3.5 mb-2 tracking-[-0.02em]'>{t('features.syncH3')}</h3>
          <p className='text-[15px] leading-relaxed'>{t('features.syncP')}</p>
          <div className='flex flex-wrap gap-2 mt-3.5'>
            {['📅 Google', '📨 Outlook', '🍎 Apple'].map((s) => (
              <span key={s} className='bg-[var(--surface-panel)] border-[3px] border-[var(--frame)] rounded-[8px] px-3 py-1.5 font-semibold text-[13px] shadow-[var(--shadow-brutal-xs)]'>{s}</span>
            ))}
          </div>
        </div>
        ...
        <div className={`${brutCard} md:col-span-2 p-7 transition-all duration-75 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[9px_9px_0_0_var(--frame)]`}>
          ...
          <h3 className='font-display font-extrabold text-[26px] mt-3.5 mb-2 tracking-[-0.02em]'>{t('features.conflictsH3')}</h3>
          <div className='flex flex-wrap gap-1.5 mb-3.5'>
            {[
              ...
              <span
                ...
                style={{ background: alert ? 'var(--color-brand-red)' : 'var(--background)', color: alert ? 'white' : 'inherit' }}
              >
                {text}
              </span>
            ))}
        ...
        >
          ...
          <h3 className='font-display font-extrabold text-[26px] mt-3.5 mb-2 tracking-[-0.02em] text-white'>{t('features.countriesH3')}</h3>
          <p className='text-white/90 text-[15px] leading-relaxed mb-3'>{t('features.countriesP')}</p>
          <div className='flex flex-wrap gap-2'>
            {['🇪🇸', '🇮🇹', '🇩🇪', '🇫🇷', '🇬🇧', '🇺🇸', '+8'].map((flag) => (
              <span key={flag} className='bg-[var(--surface-panel)] border-[3px] border-[var(--frame)] rounded-[8px] px-2.5 py-1.5 text-[13px] font-semibold shadow-[var(--shadow-brutal-xs)]'>{flag}</span>
            ))}
          </div>
        </div>
