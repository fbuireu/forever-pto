  ...
  return (
    <section className='px-7 py-24 bg-[var(--surface-panel-alt)] border-y-[4px] border-[var(--frame)]' id='pricing'>
      ...
      <div className='max-w-[1100px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-7'>
        {/* Free */}
        <div className={`${brutCard} p-8 transition-all duration-75 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[9px_9px_0_0_var(--frame)]`}>
          ...
          <div className='font-display font-extrabold text-[64px] leading-none tracking-[-0.04em] mb-1.5'>
            {t('pricing.freePrice')}<span className='text-[18px] text-muted-foreground font-semibold'>{t('pricing.freePer')}</span>
          </div>
          ...
          <ul className='list-none mb-6'>
            {Array.from({ length: 4 }, (_, i) => t(`pricing.freeFeatures.${i}` as Parameters<typeof t>[0])).map((f) => (
              <li key={f} className='flex gap-2.5 py-2 border-b-[2px] border-dashed border-black/15 last:border-b-0 text-[15px]'>
        ...
        {/* Lifetime — featured */}
        <div className={`${brutCard} p-8 relative rotate-[-1deg] transition-all duration-75 hover:rotate-0 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[9px_9px_0_0_var(--frame)]`} style={{ background: 'var(--accent)' }}>
          ...
          <h3 className='font-display font-extrabold text-[28px] tracking-[-0.02em] mb-2'>{t('pricing.lifetimeName')}</h3>
          <p className='text-[14px] text-muted-foreground mb-4'>{t('pricing.lifetimeTagline')}</p>
          <div className='font-display font-extrabold text-[64px] leading-none tracking-[-0.04em] mb-1.5'>
            {t('pricing.lifetimePrice')}<span className='text-[18px] text-muted-foreground font-semibold'>{t('pricing.lifetimePer')}</span>
          </div>
          <p className='font-mono text-[12px] text-muted-foreground mb-5'>{t('pricing.lifetimeNote')}</p>
          <ul className='list-none mb-6'>
            {Array.from({ length: 6 }, (_, i) => t(`pricing.lifetimeFeatures.${i}` as Parameters<typeof t>[0])).map((f) => (
              <li key={f} className='flex gap-2.5 py-2 border-b-[2px] border-dashed border-black/15 last:border-b-0 text-[15px]'>
                <span className='font-black'>✓</span> {f}
              </li>
            ))}
          ...
          <Button className='w-full justify-center' asChild>
            <Link href='/planner'>{t('pricing.lifetimeCta')}</Link>
          </Button>
        </div>
      </div>
    </section>
