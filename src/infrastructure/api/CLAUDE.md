...
| File | Contents |
| --- | --- |
| [`errors.ts`](./errors.ts) | `ApiError` — the machine-readable failure codes that go on the wire |
...
| `SessionError`, `DatabaseError` | 500 | `INTERNAL_ERROR` | [`src/app/api/check-session/route.ts`](../../app/api/check-session/route.ts) |
