...
Three trees are excluded from the run in [`vitest.config.ts`](../../vitest.config.ts): `assets/icons/**`, `i18n/messages/**` and `modules/bones/**`. Coverage excludes those plus `modules/core/animate/icons/` — all but `Icon.tsx`, which is tested. They are data or generated output, and a coverage number over them would be noise.
