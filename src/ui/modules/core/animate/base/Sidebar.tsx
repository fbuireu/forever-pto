...
function SidebarTrigger({ className, onClick, ...props }: SidebarTriggerProps) {
  ...
  return (
    <Button
      ...
      size='icon'
      className={cn(
        'relative h-11 w-11 rounded-[10px] bg-[var(--accent)] text-[var(--accent-foreground)] border-[3px] border-[var(--color-brand-ink)] shadow-[var(--shadow-brutal-xs)] transition-[transform,box-shadow] hover:bg-[var(--accent)] hover:text-[var(--accent-foreground)] hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[var(--shadow-brutal-sm)] cursor-pointer before:content-[""] before:absolute before:-inset-[3px]',
        className
      )}
      onClick={(event) => {
        onClick?.(event);
