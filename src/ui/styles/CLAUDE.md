...
| [`utilities/index.css`](./utilities/index.css) | `@utility hit-area-stable` and `hit-area-stable-tilt` |
...
pins the hit area while the box moves — a transparent `::after` at `inset: 0` behind the element
...
The same trick is hand-written for the driver.js buttons in `lazy/index.css` — an `@utility` cannot be
