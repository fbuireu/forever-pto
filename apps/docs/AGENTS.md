...
- Independent workspace package (`forever-pto-docs`) with its own CI ([`.github/workflows/docs.yml`](../../.github/workflows/docs.yml)) and Cloudflare Worker (static assets). It never imports app code as a workspace dependency, only raw sources. Most of that goes through the `@ui` alias, but **not all of it**. Other things reach across:
  ...
  [`tests/docs-consistency.test.ts`](../../tests/docs-consistency.test.ts) derives that whole reach from the sources and asserts every path of it appears in `docs.yml`'s `DOCS_PATHS`, so a new import that the docs workflow would not rebuild on fails the app's own suite. It scans the **whole package**, not just `src/`; [`astro.config.ts`](./astro.config.ts) and [`tsconfig.json`](./tsconfig.json) are where the seam is declared and were outside the old scope.
...
- **The logo is the app's, copied rather than referenced.** [`src/assets/forever-pto-logo.png`](./src/assets/forever-pto-logo.png) is a byte copy of `apps/web/public/static/images/forever-pto-logo.png`, the one file the app renders in both themes (what follows the theme is the frame and the shadow, through `--frame`, not the image), because Starlight's `logo` option resolves inside this package and a path out of it is not an asset the build can process. [`src/components/SiteTitle.astro`](./src/components/SiteTitle.astro) draws it the way the app's header does, in a tilted accent box that straightens on hover, beside the version chip. A new logo in the app is two files to copy here: the PNG and `public/favicon.ico`, which is the app's `src/app/favicon.ico` byte for byte.
...
- **What this package reaches into the app for is what `docs.yml` has to rebuild on.** Today that is all of `src/ui/`, plus [`src/domain/calendar/types.ts`](../web/src/domain/calendar/types.ts), [`src/domain/calendar/const.ts`](../web/src/domain/calendar/const.ts) and [`src/infrastructure/i18n/locales.ts`](../web/src/infrastructure/i18n/locales.ts), which `HowItWorksDemo` and `PlanningEngineDemo` import by relative path. Reaching for a new app module means adding its path to `DOCS_PATHS` in the `changes` job, or a rename over there merges green and breaks the site here.
...
- **Diagrams are Mermaid fences, drawn at build time.** Starlight runs on Sätteri, Astro's default Markdown processor, which takes mdast plugins rather than remark ones: [`src/lib/mermaid-plugin.ts`](./src/lib/mermaid-plugin.ts) replaces every ```mermaid fence with two `<figure>` elements, one per theme, and `astro.config.ts` hands the plugin to `satteri({ mdastPlugins })` as `markdown.processor`, which the MDX integration inherits. [`src/lib/mermaid-render.ts`](./src/lib/mermaid-render.ts) does the drawing in one headless Chromium held open for the whole build (the `@playwright/test` the suite already needs), with the site's own display face loaded so labels are measured against the glyphs a reader will see, and with the palette read out of the app's [`global/index.css`](../web/src/ui/styles/global/index.css) rather than spelled a second time; the dark block restates only what changes, so a token missing there falls back to `:root`. Results are memoised by content hash, so a diagram repeated across locales is drawn once and 500 pages cost about ten seconds.

  Four details are load-bearing. **A source Mermaid cannot parse fails the build**, naming the page, rather than shipping a frame that reads `error` in the browser. **Labels are SVG text, not `<foreignObject>` HTML** (`htmlLabels: false`): the HTML kind is measured against a page stylesheet this renderer does not have, so the layout came out scrambled, and the HTML inside the SVG is also what made the two copies nest. **The two figures are emitted as two nodes** through `ctx.replaceNode(node, [light, dark])`, not as one string holding both, for the same nesting reason. **The theme is picked in CSS**, by `figure.mermaid-dark`/`-light` rules keyed on `data-theme` with a `prefers-color-scheme` middle state, because Starlight's own `dark:sl-hidden` utility is layered and the frame's own `display: flex` outranks it.

  The client script that remains, [`src/lib/mermaid.ts`](./src/lib/mermaid.ts), is a click-to-enlarge toggle and nothing else: no Mermaid reaches the browser, there is no flash of source text before a draw, and a reader without JavaScript sees the diagram. The source rides along URI-encoded in `data-mermaid` for anyone reading the markup. Rendering in the browser was the first shape and is the one to avoid: it cost a visible rehydration on every page and put a megabyte of Mermaid in the bundle.
...
- **A docs pull request is titled for this package**: the repo squash-merges and the PR title becomes the commit on main, and attribution is by path, so a `fix:` touching only `apps/docs` cuts a docs release, never an app one. See `/contributing/conventions/`.
