---
title: Emails
description: Resend plus react-email. Which emails exist, when they fire and how the contact pipeline works.
sidebar:
  order: 15
---
...
```mermaid
sequenceDiagram
  participant U as Contact form
  participant R as /api/contact
  participant UC as sendContactEmail
  participant RS as Resend
  participant DB as Turso
  U->>R: POST { name, email, message }
  R->>UC: run with ApplicationLayer
  UC->>UC: zod contactSchema
  UC->>UC: render ContactFormEmail to HTML
  UC->>RS: send from and to the contact address, replyTo = visitor
  RS-->>UC: messageId
  UC-->>R: ok
  R-->>U: 200
  R--)DB: after(): saveContact with the messageId, failure only logs
```
...
3. **Deferred persistence**: the submission is also stored in the Turso `contacts` table (`saveContact`, `apps/web/src/infrastructure/services/contact/repository.ts`), including Resend's `messageId`. Like the payment save, this runs after the response via `next/server`'s `after()`, and a DB failure only logs, because the user's message was already delivered.
