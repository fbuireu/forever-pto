...
description: PDF and ICS exports, covering the CalendarExport component, @react-pdf rendering, the hand-rolled ICS generator and premium gating.
sidebar:
  order: 10
---
...
```mermaid
flowchart LR
  ST[(holidays + filters stores)] --> SEL[Selected Holidays and PTO Days<br/>currentSelection ?? suggestion]
  SEL --> T{Format}
  T -- PDF --> DYN[dynamic import<br/>@react-pdf/renderer + HolidayDocument]
  DYN --> BLOB1[pdf.toBlob]
  BLOB1 --> URL[Object URL, released by Effect.acquireRelease]
  T -- ICS --> GEN[generateIcs<br/>one all-day VEVENT per date, deterministic UID]
  GEN --> BLOB2[text/calendar blob]
  URL & BLOB2 --> DL[a.download = forever-pto-year.ext]
```
