...
The `next-intl` middleware (`createMiddleware(routing)` in `apps/web/src/proxy.ts`) resolves the locale per request. Unknown locales hit `notFound()` in the root layout, which renders `generateStaticParams` over all locales.
