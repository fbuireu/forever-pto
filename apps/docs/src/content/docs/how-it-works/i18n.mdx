...
- `localePrefix: 'as-needed'` means English lives at `/` and every other locale gets a prefix (`/es/...`, `/ca/...`).
...
`next-intl` persists the resolved locale in the `NEXT_LOCALE` cookie (constant `LOCALE_COOKIE` in `apps/web/src/infrastructure/i18n/config.ts`). The middleware then **re-sets** that same cookie through `setLocaleCookie()` (`apps/web/src/infrastructure/i18n/cookie.ts`) to harden it: `httpOnly: true`, `secure: true`, `sameSite: 'lax'`, `path: '/'`, and no `maxAge`, making it a session cookie. See [cookies](/reference/cookies/).
