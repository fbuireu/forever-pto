...
The four `quick_start_*` events are the homepage funnel: `opened` carries the call to action that opened it (`nav`, `hero`, `pricing` or `closing`), `step_completed` and `abandoned` carry the step (`location`, `ptoDays` or `settings`), and `completed` carries the Country, the Region, the year, the Strategy, past days and Carry-over Months. It never carries the PTO Day budget, which is the one planning input the [data sources](/how-it-works/data-sources/) page promises stays in the browser.

The planner and the rest of the app report these, under the same rule: a setting, a feature id, an outcome or a count, never a date, a name or the budget.

| Event | Fires when | Carries |
| --- | --- | --- |
| `planner_generated` | The worker settles a plan (every replan) | Strategy, year, Carry-over Months, past days, the number of Alternatives and the plan's quality: Efficiency, Effective Days, Bonus Days, Long Weekends, Rest Blocks, Longest Vacation, Max Work Streak |
| `planning_input_changed` | A sidebar control changes a planning input | `input` (`country`, `region`, `year`, `ptoDays`, `strategy`, `allowPastDays`, `carryOverMonths`) and its new value, except the budget, which reports only a `direction` (`up` or `down`) and, from the calculator, a `source` |
...
| `tool_used` | A calculator produces a result | `tool` (`ptoCalculator` or `workdayCounter`) |
...
The salary calculator reports nothing: its only inputs are a salary and a count of PTO Days left, and neither is a setting.
