...
The four `quick_start_*` events are the homepage funnel: `opened` carries the call to action that opened it (`nav`, `hero`, `pricing` or `closing`), `step_completed` and `abandoned` carry the step (`location`, `ptoDays` or `settings`), and `completed` carries the Country, whether a Region was chosen, the year, the Strategy, past days and Carry-over Months. It never carries the PTO Day budget, which is the one planning input the [data sources](/how-it-works/data-sources/) page promises stays in the browser.
...
| `calendar_day_toggled` | A day is clicked | `applied`, or `applied: false` with the store's refusal (`budget_exhausted`, `day_is_holiday`…) or `premium_required` when the gate stopped it |
