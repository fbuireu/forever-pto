...
| --- | --- | --- |
| `planner_generated` | The worker settles a plan (every replan) | The PTO Day budget, Strategy, year, Carry-over Months, past days, the number of Alternatives and the plan's quality: Efficiency, Effective Days, Bonus Days, Long Weekends, Rest Blocks, Longest Vacation, Max Work Streak |
| `planning_input_changed` | A sidebar control changes a planning input | `input` (`country`, `region`, `year`, `ptoDays`, `strategy`, `allowPastDays`, `carryOverMonths`) and `inputValue`, its new value; the budget applied from the calculator adds `source: ptoCalculator`. The property is not called `value` because GA4 reserves that name for the monetary value of a key event |
