...
`apps/web/src/infrastructure/clients/logging/better-stack/tracking.ts` exposes `track({ event, properties })` and `identifyUser({ email, plan })` over `window.betterstack`. Event names are a closed union (`TrackEventName`): `payment_started`, `payment_completed`, `payment_failed`, `promo_code_applied`, `premium_activated`, `upgrade_modal_opened`, `feature_unlocked`, `planner_generated`, `contact_form_submitted`, the four `quick_start_*` events and the planner set below, so a typo or ad-hoc event is a compile error. Both functions fan out to two destinations: the Better Stack script, where an event sits next to the logs and the errors of the same release, and Google Analytics through `gtag`, where the same event name and properties feed funnel and path explorations, key events and audiences. Each no-ops on its own when its script is absent (SSR, consent denied); GA's consent mode decides on its side what a denied visitor's event becomes. The email goes to Better Stack only; GA receives the plan as a user property and nothing that identifies the person. `upgrade_modal_opened` carries the gate's `feature` and an `origin`, `planner` unless the gate sits in the homepage quick start, which reports `quick_start`.
...
The four `quick_start_*` events are the homepage funnel: `opened` carries the call to action that opened it (`nav`, `hero`, `pricing` or `closing`), `step_completed` and `abandoned` carry the step (`location`, `ptoDays` or `settings`), and `completed` carries every planning input the visitor finished with: the PTO Day budget, the Country, the Region, the year, the Strategy, past days and Carry-over Months.

The planner and the rest of the app report these, under the same rule: a setting, a feature id, an outcome or a count, never a date or a name. The budget is a setting and travels as its number; the dates a plan places, the Manual Days and the Custom Holidays never do, which is what the [data sources](/how-it-works/data-sources/) page promises.

| Event | Fires when | Carries |
| --- | --- | --- |
| `planner_generated` | The worker settles a plan (every replan) | The PTO Day budget, Strategy, Preferred Months (as a comma-separated list), year, Carry-over Months, past days, the number of Alternatives and the plan's quality: Efficiency, Effective Days, Bonus Days, Long Weekends, Rest Blocks, Longest Vacation, Max Work Streak |
| `planning_input_changed` | A sidebar control changes a planning input | `input` (`country`, `region`, `year`, `ptoDays`, `strategy`, `preferredMonths`, `allowPastDays`, `carryOverMonths`) and `inputValue`, its new value (the Preferred Months as a comma-separated list); the budget applied from the calculator adds `source: ptoCalculator`. The property is not called `value` because GA4 reserves that name for the monetary value of a key event |
| `calendar_day_toggled` | A day is clicked | `applied` with what changed (`manual_day_added`, `manual_day_removed`, `suggested_day_removed`, `removed_day_restored`), or `applied: false` with the store's refusal (`budget_exhausted`, `day_is_holiday`…) or `premium_required` when the gate stopped it |
| `alternative_applied` | An Alternative becomes the Suggestion | Its index and its Efficiency and Effective Days |
| `manual_changes_reset` | The manual changes are discarded | `surface` (`panel` or `sidebar`) |
| `holiday_tab_changed` | A Holiday table tab is opened | The Holiday Variant of the tab |
| `holiday_selection_changed` | A Holiday is picked or un-picked, or select-all is used | The Holiday Variant, `scope` (`one` or `all`) and `selected` |
| `holidays_sorted` | A Holiday table column is sorted | The Holiday Variant and the column `key` |
| `holidays_searched` | A search in a Holiday table starts (once per search, never the text) | The Holiday Variant |
| `holiday_modal_opened` | The add, edit or delete button of a Holiday table is pressed | `action` (`add`, `edit`, `delete`) and the tab's Holiday Variant |
| `custom_holiday_saved` | A Custom Holiday is added or edited | `mode` (`add` or `edit`), `applied` and the refusal reason if any |
| `custom_holiday_deleted` | Custom Holidays are deleted | `count` |
| `calendar_exported` | An ICS or PDF export finishes | `format`, `includeHolidays`, `includePto`, `outcome` (`success` or `error`) |
| `donate_opened` | The donation popover opens | `source` (`floating`, `planner_toast` or `pricing`) |
| `payment_cancelled` | The checkout is abandoned back to the form | nothing |
| `tool_used` | A calculator produces a result | `tool` (`ptoCalculator`, `ptoSalaryCalculator` or `workdayCounter`) |
| `tutorial_started` | The guided tour starts | `isMobile` |
| `language_changed` | The language switcher is used | `from`, `to` |
| `theme_changed` | The theme selector is used | `theme` |
| `contact_opened` | The feedback form opens | `source` (`click` or `hash`) |

The salary calculator reports only that it was used, once per session of figures: the salary is pay data and stays in the browser.
