...
| `planning_input_changed` | A sidebar control changes a planning input | `input` (`country`, `region`, `year`, `ptoDays`, `strategy`, `allowPastDays`, `carryOverMonths`) and its new value; the budget applied from the calculator adds `source: ptoCalculator` |
