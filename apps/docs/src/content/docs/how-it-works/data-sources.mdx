...
The plan itself, the Manual Days, the Custom Holidays. None of it leaves the browser unless the user exports it. Analytics events (`payment_completed`, `upgrade_modal_opened`, `quick_start_completed` and the rest) carry an amount, a feature id, a setting, an outcome or a count; the PTO Day budget travels as one of the settings, a date or a Holiday name never does; see [observability](/how-it-works/observability/).
