...
The plan itself, the Manual Days, the Custom Holidays, the budget. None of it leaves the browser unless the user exports it. Analytics events (`payment_completed`, `upgrade_modal_opened`, `quick_start_completed` and the rest) carry an amount, a feature id, a setting or an outcome, never a date and never the budget; see [observability](/how-it-works/observability/).
