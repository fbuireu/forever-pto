...
The values are obfuscated with a build-time key before they hit storage (XOR plus base64), which keeps casual tampering out and is not cryptography; [ADR 0007](https://github.com/fbuireu/forever-pto/blob/main/adr/0007-persisted-client-state-is-obfuscated-not-encrypted.md) explains why that is the right amount. Details on the [state stores](/how-it-works/state-stores/) page.
