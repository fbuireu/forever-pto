...
1. The other two Strategies' plans come first, over the same candidates. They are the most different plans the engine can make, and the user sees what the other objectives would have done.
...
Every Alternative is stamped with the chosen Strategy, because applying it makes it the Suggestion. Two runs over the same inputs produce the same Alternatives, so the user's "option three" is still option three after a refresh.
