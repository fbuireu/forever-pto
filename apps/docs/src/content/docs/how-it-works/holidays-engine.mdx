...
The seam used to sit one level higher, with the filter inside the production adapter only; the fixture then had to hand-write the filtered shape, and deleting the filter left the end-to-end test green. `apps/web/src/infrastructure/services/holidays/AGENTS.md` records that move.
