...
sequenceDiagram
  ...
  WK->>WK: deserialise: fromStoredInstant, isFilterStrategy, isPreferredMonths, isLocale
  WK->>PL: PlanningInput
...
| `allowPastDays`, `strategy`, `preferredMonths`, `locale` | Filters | `strategy`, `preferredMonths` and `locale` are **narrowed** on arrival, with `isFilterStrategy`, `isPreferredMonths` and `isLocale`, falling back to `DEFAULT_FILTER_STRATEGY`, no preferred month and English, because this is the inbound leg where a value out of persisted storage arrives |
...
`apps/web/src/infrastructure/workers/worker.test.ts` stubs `self`, mocks `runPlanningPipeline` and drives `globalThis.onmessage` directly. It pins the boundary and only the boundary: the message-type guard, deserialisation into the pipeline's input names, the two narrowings, serialisation of the reply, that an unplanned result reaches the wire with the pipeline's own `Metrics`, and that a throw becomes `WORKER_ERROR`. The pipeline's behaviour is pinned once, against the real engine, in `apps/web/src/domain/calendar/pipeline.test.ts`. `apps/web/src/infrastructure/workers/AGENTS.md` is the folder's own guide and carries the invariants in full.
