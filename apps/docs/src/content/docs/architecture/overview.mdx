...
Forever PTO is a Next.js App Router application organized into five layers under `apps/web/src/`, plus `apps/web/src/proxy.ts` at the root of the tree. Each layer has a single responsibility and a deliberate dependency direction.
...
`proxy.ts`, and the old figure drew five of them. The largest one it left out was `app → infrastructure`, at
