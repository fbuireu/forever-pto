...
description: The layered src/ architecture (app, application, domain, infrastructure and ui) plus dependency rules, nested AGENTS.md guides and path aliases.
sidebar:
  order: 1
---
...
- **`application → ui` is a single import**, recorded as a known inversion in
  `apps/web/src/application/AGENTS.md`. One is the number; a second is a regression.

The normative statements stay with the code, in the layer contracts, and this page links rather than restates
them:

- `apps/web/src/domain/AGENTS.md` enumerates exactly what the pure calendar context may import from outside
  itself, and why the test for a fifth entry is the runtime rather than the layer.
- `apps/web/src/application/AGENTS.md` carries the orchestration rules and the one `@ui/*` inversion.
- `apps/web/src/infrastructure/AGENTS.md` carries what the outward-facing layer may reach back into.
- `apps/web/src/ui/AGENTS.md` lists the short set of pure value helpers the UI may take from
...
Per the repo's root `AGENTS.md`, every significant folder carries its own `AGENTS.md` describing what may be changed there and what its rules are; read it before touching the folder. Examples: `apps/web/src/domain/AGENTS.md` (layer rules and allowed imports), `apps/web/src/infrastructure/clients/AGENTS.md` (the Effect client pattern), `apps/web/src/application/stores/AGENTS.md` (store conventions). Those files are the source of truth for layer boundaries; this page summarizes them. `CONTEXT.md` is a different document and exists **only** at the repo root, where it is the domain glossary. A nested one is forbidden, because the name would be ambiguous.
