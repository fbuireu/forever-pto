...
The [overview](/architecture/overview/) measures the dependency graph; this page walks each node of it. Every layer carries a `AGENTS.md` at its root that is the binding contract; this page is the tour, and it links the contract rather than restating it where the contract is long.
...
`apps/web/src/domain/calendar/AGENTS.md` is the engine's own guide, with its public API, invariants and the traps a change falls into.
...
- **`core/` imports nothing back.** No stores, no `useTranslations`, no fetching; components take strings and callbacks. Everything else may import `core/`; where a new component goes is a five-question walk in `apps/web/src/ui/modules/AGENTS.md`.
...
- **A component opts out of the server render with `use(browser())`; everything else keeps its `typeof window` guard.** React added `browser()` to `react-dom`, so a browser-only component suspends on the server instead of proving at every access that `document` exists, inside a `<Suspense>` boundary, which is not optional. Module scope, plain utilities and event handlers cannot call `use()` and keep the guard, because the bare identifier still throws there.
...
The design tokens, the shadow scale and the cascade order are the [design system foundations](/design-system/foundations/philosophy/); `apps/web/src/ui/styles/AGENTS.md` explains why line one of `index.css` must stay above the Tailwind import.
