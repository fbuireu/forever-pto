---
title: Proxy
description: The per-request chain in src/proxy.ts (markdown negotiation, payment guard, i18n, locale cookie and country detection).
sidebar:
  order: 3
---

`apps/web/src/proxy.ts` is the single per-request entry point: what Next called middleware until 16, running on the Node.js runtime under its new name. The old `/architecture/middleware/` address redirects here. It runs a short, strictly ordered chain and returns as early as it can. Each step delegates to a focused helper; the proxy file itself is orchestration only.
...
The proxy imports nothing framework-agnostic itself; the actual logic (`detectCountry`, cookie writers) lives under `apps/web/src/infrastructure/proxy/` and `apps/web/src/infrastructure/services/location/`, keeping `apps/web/src/proxy.ts` a readable table of contents for the request pipeline and keeping the helpers unit-testable (`apps/web/src/proxy.test.ts` covers the orchestration).
