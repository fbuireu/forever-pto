...
4. **`apps/docs` runs commit-analyzer, release notes and GitHub only.** No changelog, no npm, no git plugin: it pushes nothing to `main`, which is what keeps the release jobs from competing for the branch. Its own version stays `0.0.0` and nothing reads it: the site displays the **app's** version.
