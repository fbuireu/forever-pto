...
6. **A commit landing on `main` mid-release loses that release, and a dispatch recovers it.** `@semantic-release/git` pushes the version bump as `HEAD:main` from the sha the job checked out; a commit that reached `main` in the meantime makes the push a non-fast-forward, the job fails after the deploy and the smoke run passed, and no tag is left behind. Re-running the job does not help either, because semantic-release stands down when the branch it holds is behind the remote. Both workflows take a `release` input on `workflow_dispatch`, off by default: checked, the dispatch redeploys the head of `main`, runs the smoke cases and then the release job from a fresh checkout, so the version is computed over every commit since the last tag, the refused one included. It happened on 2026-09-06, and until the input existed the recovery was a filler commit touching `WEB_PATHS`.
