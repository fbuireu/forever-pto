...
| `apps/docs` | `docs-vX.Y.Z` | `package.json`, `CHANGELOG.md`, a GitHub Release | `docs.yml`, after the docs deploy and its smoke run |
...
1. **A release means the version is live.** `release-web` `needs: deploy-production`, so nothing is tagged until the deploy has succeeded. `release-docs` sits behind the docs deploy and its smoke run for the same reason. Both share the concurrency group `release` with `cancel-in-progress: false`, so they queue instead of racing. Queueing alone was not enough: the checkout pins the run's sha, so the second job held `main` one release commit behind the remote and semantic-release stood down, green, on every push that touched both packages. Each release job now fast-forwards onto `origin/main` first, whatever landed since, so the sibling's release commit and any merge that arrived mid-run both join the release.
...
6. **A commit landing on `main` mid-release joins that release.** `@semantic-release/git` pushes the version bump as `HEAD:main` from the branch the job holds, and the checkout pins the run's sha, so a commit that reached `main` in the meantime used to make the push a non-fast-forward: the job failed after the deploy and the smoke run passed, no tag was left behind, and a re-run stood down because the branch it held was behind the remote. The fast-forward in point 1 is what changed: the version now covers every commit on `main` when the job runs, and the run those commits queued has nothing left to publish. A tag can therefore precede the deploy of the commits it absorbed by a few minutes. The two cases that still stand a job down, `main` rewritten under the run or a merge in the seconds between the fast-forward and the push, write no tag, so the run the newer head queued cuts the release over everything since the last one. It happened on 2026-09-06, and until now the recovery was a filler commit touching `WEB_PATHS`.
...
- The old rule requiring documentation PRs to use `docs:` titles is gone; it existed only to stop single-package versioning doing the wrong thing. The `semanticCommitType: docs` rule in `renovate.json` for `apps/docs/package.json` stays, so a docs dependency bump does not read as an app change; attribution already books it against `apps/docs`.
