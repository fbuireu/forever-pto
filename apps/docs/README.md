...
the package the importing file sits in, so a filtered install leaves `apps/web/node_modules` absent and the
...
Read [`CLAUDE.md`](./CLAUDE.md) before changing anything: it carries the boundary rules, namely which app
