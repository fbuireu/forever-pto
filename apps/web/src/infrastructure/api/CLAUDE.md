...
| File | Contents |
| --- | --- |
| [`errors.ts`](./errors.ts) | `ApiError`, the machine-readable failure codes that go on the wire, plus `describeFailure`, the one tag→status table |
...
in, and `api/check-session`'s `POST` passed a program with **no limiter at all**, on an endpoint that grants
...
, since `'unknown'` from one transport and `null` from the other is the defect recorded below.
...
therefore does not compile, and the handler destructures it *before* the guard. That is not a style choice, and
