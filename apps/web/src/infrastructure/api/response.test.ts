...
describe("noStore", () => {
	it("returns a JSON response with the given body", async () => {
		const response = noStore({ body: { success: true } });
		const body = await response.json();
		expect(body).toEqual({ success: true });
	});

	it("sets Cache-Control: no-store header", () => {
		const response = noStore({ body: { ok: true } });
	...
	it("forwards a custom status code", () => {
		const response = noStore({ body: { error: "not found" }, init: { status: 404 } });
		expect(response.status).toBe(404);
	});

	it("forwards custom headers alongside Cache-Control", () => {
		const response = noStore({ body: { ok: true }, init: { headers: { "X-Custom": "value" } } });
