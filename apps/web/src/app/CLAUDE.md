...
| Path | Role |
| --- | --- |
| `[locale]/layout.tsx` | **The root layout.** There is no layout file at the `src/app` root, so this is the only file that renders `<html>`/`<body>` on the normal path |
...
fallback to the query, because the fallback is the vulnerability, not a convenience. `middleware.test.ts` asserts
...
oversight.** The app itself is safe, because the middleware runs before any cache lookup, so a markdown request is
...
`[locale]/layout.tsx` re-validates the segment with `hasLocale` and calls `notFound()`, because the middleware is
...
`legal/` pages pass the locale explicitly, through `getTranslations({ locale, namespace })`, so they never read the
...
could omit it and get `canonical: localePath(locale, undefined)`, that is `'/'`, and the marketing page *did*,
...
| `api/webhooks/stripe/route.ts` | POST | Verifies the `stripe-signature` header, then hands the event to `processWebhookEvent`. Reads the **raw** body via `request.text()`, since parsing it as JSON would break signature verification |
...
- The Effect program is run at the boundary, as `Effect.runPromise(program.pipe(Effect.provide(ApplicationLayer)))`,
...
- A JSON body is read with `parseJsonBody` (`api/parseJsonBody.ts`) **inside** the Effect program, never with
  ...
  the exception, because it needs the raw text.
...
renders `premiumActivationFailed`, so the payer is told their money went through and their access did not,
...
over 24 hours old, which is false for any donor who opened the planner before donating, since `PremiumFeature`'s own
...
`checkExistingSession({ force: true })` once; it activates nothing, since the cookie is already set, server side,
...
answered `/.well-known/constructor`, and `toString`, `valueOf` and `hasOwnProperty`, with an inherited
...
These paths contain a dot, so the middleware matcher excludes them and they never see locale negotiation.
...
| File | Catches |
| --- | --- |
| `[locale]/(marketing)/error.tsx` | Errors in marketing pages. Renders inside the marketing layout, so it emits bare content, because header and footer are already there |
...
`metadata.test.ts`, seven times over, and the four `legal/` pairs differed from each other on exactly two lines
...
route's keys for another's keeps it green, verified by doing exactly that.
 What catches it is a property the
...
`isIndexable` **fails closed**, treating a path with no row as private, so the failure mode of forgetting
...
a Donation the payer chooses, and `src/application/dto/payment/schema.ts` accepts 1 to 10000, so the fixed
...
the exceptions, since neither has behaviour worth asserting.
