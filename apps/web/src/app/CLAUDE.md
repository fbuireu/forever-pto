...
| [`fonts.ts`](./fonts.ts) | The `next/font/google` families, exported as CSS-variable handles |
...
2. Rewrites any request carrying `Accept: text/markdown` to `/api/markdown`, passing the pathname in an
   ...
   **The lookup is an exact match on the locale-stripped path, and that is load-bearing.** It was
   `pathname.includes(path)`, which once handed every unlisted path the homepage body: the legal pages
   ...
   **The claim that used to end this paragraph (that the homepage is "the empty path matched deliberately
   ...
   homepage body, so *any* path `findRoute` did not know reached it, and the suite pinned exactly that:
   ...
   **404**. The representations of one URL agreed on nothing before: `/does-not-exist` returned the
...
homepage**: `/planner`, `/es/planner` and every legal page answered 200 with the marketing copy under
...
200, `text/markdown` and `public, max-age=3600`: the things the route refuses on exactly that path,
...
`markdownTwinHeaders({ found })`: content type, `Cache-Control` and `Vary` together. It was written across
modules that each carried their own copy of `max-age=3600`, and the proxy's copy was applied *before*
...
URLs carry **no** `/en` prefix while the others do. Anything building a URL must go through
...
decoration. Some files do not call it, and each has a reason rather than an oversight: the
...
mounts none.** It used to open a second one inside this one, so the tree carried a second independent
`open`/`openMobile` state and nested `div.flex.min-h-svh.w-full` wrappers. Every consumer that
...
between them, would have read the dead one. `AppSidebar` returns a fragment now and takes the context
...
There were sibling `metadata.ts` files, each a few lines long (an import of `routeMetadata` and that
same export) plus an `export { generateMetadata } from './metadata';` edge re-exporting each one. The split
...
Rules are derived rather than repeated: `openGraph` appears when there is a description, and
`images`/`twitter`/`keywords` only when the route is indexable, which is exactly what those files did
by hand.

**`indexable` used to be a parameter, and every caller computed it the same single way.**
...
They were copies of one long shape; the `legal/` ones were identical bar a namespace and a
repeated path. The translation call stays per-route on purpose: `getTranslations` types its
keys against the namespace, so resolving the strings at the call site is what keeps a typo in a message key a
compile error instead of a runtime blank. A new route that skips the `alternates` block ships a URL per locale
...
- `check-session` and `health` respond through `noStore` ([`src/infrastructure/api/response.ts`](../infrastructure/api/response.ts)). Anything
  ...
  mock is gone and every branch asserts `no-store`; verified by returning a bare `NextResponse.json` from
...
conditions `verifySession` can hit. It was one branch for all of them: an expired token, a bad signature, a rotated
...
`LoggerService` tag like every other route's. Every Live layer is `Layer.sync` and reads no environment
...
- **The client secret is required and verified, and that is now true of the *type*, not just of this
  ...
  no guard at all. It is one entry point per caller now:
...
against exact keys (`api-catalog`, `mcp/server-card.json`, `agent-skills/index.json`), delegating
...
the prototype-key cases go red.
...
`agentSkillsIndex.ts` used to give every entry a `url` under `/.well-known/agent-skills/<name>/SKILL.md`
and a `sha256`: every one of those URLs 404s, because the handler matches exact keys and none of them
is a `SKILL.md`, and every digest was `e3b0c442…`, the SHA-256 of the empty string. The entries
describing a document that does exist now point at it; those describing a *behaviour* carry no `url` at
...
is a value now: [`well-known/slugs.ts`](../infrastructure/well-known/slugs.ts) holds the slugs and `wellKnownUrl`, `well-known/documents.ts`
...
**The document modules return plain objects; the route owns the envelope.** Each used to build its own
`NextResponse.json(...)` with its own `Content-Type` and the shared `Cache-Control`, so a new document
...
`WELL_KNOWN_MISSING_CACHE_CONTROL`, so both branches are one decision.

This is the same defect the Markdown twin had, in the same shape, recorded above: a hit with an explicit
...
**Adding a route is one row and one line.** It used to be one row, a `metadata.ts` and a
`metadata.test.ts`, once per route: the `legal/` pairs differed from each other on barely a line
...
That used to be an edit in each of several files (a `ROUTES` entry, a `DISALLOWED_PAGES` prefix, and
...
the document-rendering files do this (`[locale]/layout.tsx`, `global-error.tsx` and `global-not-found.tsx`), and a new
...
mock is gone and the real `noStore` runs, which is what makes the body assertions able to fail.
