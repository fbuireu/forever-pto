...
| `base/index.css` | `@layer base`, holding element defaults: border/outline colour, body background and glow, scrollbar styling, the shared transition on buttons and shadcn slots |
| `theme/index.css` | `@theme inline`, which bridges the design tokens into Tailwind's namespaces; also the `dark` and `hover` custom variants |
