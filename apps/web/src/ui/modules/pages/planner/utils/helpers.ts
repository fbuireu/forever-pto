...
export interface CalculateWorkdaysParams {
	range: FromTo;
	holidays: HolidayDTO[];
}

export function calculateWorkdays({ range, holidays }: CalculateWorkdaysParams) {
...
export interface CalculateHolidaysInRangeParams {
	range: FromTo;
	holidays: HolidayDTO[];
}

export function calculateHolidaysInRange({ range, holidays }: CalculateHolidaysInRangeParams) {
	const days = eachDayOfInterval({
		start: range.from,
		end: range.to,
	});
