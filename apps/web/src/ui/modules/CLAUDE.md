...
1. Is it a stateless visual element with no product vocabulary in it, such as a button, a field, a badge, an
...
planner itself is client-side end to end; see [ADR 0001](../../../../../adr/0001-planner-runs-in-the-browser.md).
...
- **It reported one failure twice**, through `form.setError('email', …)` *and* the ERROR
...
- `bones/*.bones.json` holds captured DOM shapes, regenerated by `pnpm bones:build`. Output path and
  colours come from [boneyard.config.json](../../../boneyard.config.json).
- [`bones/registry.ts`](./bones/registry.ts) is generated and carries a "do not edit" banner. It registers every bone under a
...
- `providers/BonesProvider.tsx` is imported by the locale layout and renders `null`. It side-effect imports
...
`showFallback ? fallback : children`, and `fixture` appears nowhere in that path. It is build-time only:
...
- `src/ui/modules/bones/**` is excluded from both the test run *and* the coverage report. It is
  generated data; asserting on it would only assert that the generator ran.
- `src/ui/modules/core/animate/icons/` is excluded from the **coverage report** only, and the glob spares
...
instead of using the callbacks it already registers. That is plausible, but the callbacks fire synchronously with the
...
Stack on therefore granted `analytics_storage` and fired a `page_view`, with Google Analytics writing `_ga` for a
...
`cc:showPreferences`, and `CookieConsent`'s handler set `showPreferences` without clearing `showBanner`, so the
...
**Anything the render reads has to be state, not a ref, because React bails out of equal updates.**
...
unstyled, since it carried the full frame and shadow, so it moved up to `core/animate/base/` rather than getting
a wrapper. Promote what a feature needs; do not reach in.

`tutorial/DriverStyles.tsx` renders `null` and exists solely to make its CSS import lazy, and [`useTutorial.tsx`](../hooks/useTutorial.tsx)
...
`as never` on the lookup key is next-intl narrowing its keys to the literals present in the bundle, while
