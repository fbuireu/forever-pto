...
- `metadata` is read through `getTranslations({ locale, namespace: 'metadata' })` in several places, none of
  them a `'use client'` component: the route `metadata.ts` files, [`buildMarkdownPage.ts`](../../infrastructure/markdown/buildMarkdownPage.ts) under
  ...
  JSON-LD. Renaming a `metadata` key means checking every one of them: miss the last and the structured data or
  the Markdown twin degrades silently, with every page still rendering correctly.

## Conventions

- Keys are camelCase, with deliberate snake_case exceptions, all of them for the same reason: the key
  *is* a machine code, looked up by a value that arrives from elsewhere, so renaming it breaks the lookup
  ...
  in [`shared/utils/helpers.ts`](../modules/shared/utils/helpers.ts). Do not "fix" any of them to camelCase.
...
- **Never write a string in ALL CAPS, and [`tests/docs-consistency.test.ts`](../../../../../tests/docs-consistency.test.ts) now checks it.** If an element must render
  ...
  readers and unfixable per locale. Keys shouted in every bundle and the rule had nothing behind it:
  ...
  (French escalated to a whole phrase) and now marks it with `<b>`, the rich-text tag the bundles already use.
...
- **`a11y` holds accessible names, and it is the only namespace that does.** More than one kind lives here. Names a
  ...
  There was a second namespace called `accessibility` holding exactly those trailing keys, and this file did
  not mention it, so an author asking "where does my `aria-label` go" had rival plausible answers and no way
...
- **The shared validation messages live in `validation.email` because more than one form needs them.** `invalid` and
  `required` were written out in `validation.contact`, `validation.payment` **and** `premiumModal`: a copy per
  form in every locale. **Some had already drifted**: Catalan and French `validation.payment`
  said "a valid email is required" where every other copy, and the English, say "enter a valid email
  address". The imperative form is canonical and the payment copies were replaced, so merging fixed the
...
- **A sentence is one key. A component may not assemble one out of several.** Keys under
  `summary.notifications` were glued together at the render site: `canImprove.message` + a number +
  `moreDays` + `toYourPlan`, and the same shape for `manualAdjustments.*` and `customHolidays.*`. Things
  ...
  other than *one* and *other*, and none could reorder: German had to smear one verb across separate fragments
  ...
  used. `manualAdjustments` is a message per shape rather than one, because the banner has shapes for added,
...
- **A rich-text tag can hold the number, which is how an animated counter survives the move to ICU.** Those
  ...
  `manualAdjustments.addedAndRemoved` carries a count each way and therefore a tag each, `a` and `r`. Tags nest, and
...
- **No currency symbol in any message, and [`../utils/currencies.test.ts`](../utils/currencies.test.ts)
  asserts it over the whole catalogue.** Nearly every locale writes `1 €` and only `en` writes `€1`, so a
  ...
  one. The rule used to be one assertion on `toasts.promoSavedDescription` alone, and other keys carried
  a glyph the whole time it was green; it walks every bundle now. `homepage.stats.plansValue` was the same
  defect without the symbol, a hand-abbreviated `12k+` written a different way in each bundle, and takes
...
- `en.json` is the reference. Add a key there first, then to the others in the same commit.
- **A string that describes an icon is coupled to the module that draws it, and moves with it.**
  ...
  said **llamp** (a lightning bolt) and carried a `⚡` besides, so one locale described a badge the
  app has never rendered. Nothing catches this: key parity compares key sets, and no test reads an icon
  name out of a sentence. When you change an icon, grep every bundle for its old name; when you write
  ...
  **Catalan was not the only one, because fixing the locale that was reported is not the same as checking
  the others.** Italian said *icona stellata* and French *marquée d'une étoile*, both a star, on the same
  string; they say *icona a scintilla* and *marquée d'une étincelle* now. German's *Funkelsymbol* was right
  all along. Several bundles described an icon the app does not draw, and the earlier fix looked complete
...
- **The product addresses the user informally, in every locale.** `du` in German, `tu` in French, `tú` in
  Spanish, and the same throughout Catalan and Italian. Some strings did not: the calendar-export toast
  ...
  against a named allow-list. Some kinds of hit are legitimate and are listed there by key path: the
  ...
  list only after checking which kind you have.
...
them say Alternative, Effective Day, Suggested Day and the selected dates, in every language. That split is deliberate: a key is an
identifier no user reads, renaming one means
editing every bundle and every call site, and `tests/docs-consistency.test.ts` asserts key parity across them
all, so a half-finished rename fails the suite rather than the eye. Rename a key only as its own change, every
bundle at once. **A translated string is different**: it is the product speaking, and it uses the
glossary's word.
...
**A key nothing reads still ships, and nothing fails when it stops being read.** A batch was found orphaned at
once, dozens of strings across the bundles: the whole `command` namespace, whose only key
...
some FAQ entries. Among those, `faq.sections.general.howWorks` and `restartTutorial` were a `question`
with no `answer`: half-deleted, and key parity could not see it because every bundle was half-deleted
...
section now, in both, because the lists are written out by hand and an entry added to one and not the
