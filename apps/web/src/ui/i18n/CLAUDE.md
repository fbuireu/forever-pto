...
**Every bundle has exactly the keys `en.json` has**, with no missing keys and no leftovers. This is asserted
...
active locale, about 93 KB of JSON for `en` and more for the others. There is no per-route splitting.
