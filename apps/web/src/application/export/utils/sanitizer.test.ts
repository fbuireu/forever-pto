...
describe("contentLine", () => {
	it("joins the property name to its value", () => {
		expect(contentLine("SUMMARY", "New Year")).toBe("SUMMARY:New Year");
	});
	...
	it.each([
		...
		expect(contentLine("SUMMARY", `a${character}b`)).toBe(`SUMMARY:a${escaped}b`);
	});

	it.each([["\n"], ["\r"], ["\r\n"]])("turns a pasted line break into the literal \\n", (breakChar) => {
		expect(contentLine("SUMMARY", `a${breakChar}b`)).toBe(String.raw`SUMMARY:a\nb`);
	});

	it("leaves a line at or under 75 octets unfolded", () => {
		const line = contentLine("SUMMARY", "x".repeat(67));
	...
	it("folds a longer line onto continuation lines a strict parser will accept", () => {
		const line = contentLine("SUMMARY", "x".repeat(200));
	...
	it("counts octets rather than characters, so multi-byte names fold correctly", () => {
		const line = contentLine("SUMMARY", "é".repeat(80));
