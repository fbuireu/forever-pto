...
| `shared/utils/` | `dates.ts` for calendar arithmetic, comparison and formatting; `dateIntake.ts` for the two ways a date arrives from outside; `zodParse.ts` for Zod validation lifted into an Effect that fails with `ValidationError`; `collate.ts` for `collateByLabel`, the one place a localised option list is ordered | `dates.ts`, `dateIntake.ts` and `collate.ts` both, `zodParse.ts` server |
...
- **A date arriving from outside goes through `dateIntake.ts`, and which function you want is a question
  ...
  shipped: `date-holidays` emits its Islamic-calendar entries with an explicit offset, as in
...
- **`formatDate` understands exactly the patterns in its map, and the compiler now says so.** `format` is
  ...
  **This is also the only place an `Intl.DateTimeFormat` is constructed and memoised.** Six files had routed
  around the whitelist and rebuilt both halves, with five private `Map<string, Intl.DateTimeFormat>` caches plus
...
schema factories in `dto/` instead, `createContactSchema` and `createPaymentSchemaWithMessages`, which take
...
pasted CRLF left a bare carriage return inside a value. The fold counts octets rather than characters, and
