...
- **"Verified" is not one thing, so do not read the guard off the wrong entry point.** `GET /api/payment/activate` verifies a client secret and no email; `POST /api/check-session` with a `premiumKey` verifies an email and no client secret. Adding the missing guard to either is a behaviour change, not a hardening: the redirect path has no email to check, and the claimed-payment path has no secret to check. The shape is recorded alongside the code in [`../apps/web/src/application/use-cases/AGENTS.md`](../apps/web/src/application/use-cases/AGENTS.md).
