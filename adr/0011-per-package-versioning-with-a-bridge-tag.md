...
Accepted. Amended 2026-09-23: the second release job in the `release` group fast-forwards over the first one's release commit, because the group serialised the jobs without moving the checkout and the second one stood down on every cross-package push; the *Decision* section carries it. Amended 2026-09-12: `apps/docs` now runs the same plugin chain as `apps/web` and pushes its own
...
- **A commit landing on `main` while a release job runs loses that release.** `@semantic-release/git` pushes the release commit as `HEAD:main` from the sha the job checked out, so a squash merge in the few minutes between the deploy going green and the push makes it a non-fast-forward: the job fails, no tag is written, and a re-run stands down because the branch it holds is behind the remote. The `release` concurrency group orders the two release jobs against each other and nothing else; it cannot order a merge. Both workflows therefore take a `release` boolean on `workflow_dispatch`, off by default, so the recovery is a dispatch on `main` that redeploys, smokes and releases from a fresh checkout, rather than a filler commit whose only content was the need to re-trigger the job. The rule stays: queue a merge behind a running release, not beside it.
