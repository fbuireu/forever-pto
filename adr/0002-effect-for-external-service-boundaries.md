...
These shapes are deliberate, and the first of them contradict the one-line version of this rule:
...
**Some browser files use Effect**, and the list below is exact rather than approximate, because the point of writing it down is that a new one needs a reason. [`ui/adapters/payments/checkout.ts`](../apps/web/src/ui/adapters/payments/checkout.ts) orchestrates the fallible Stripe and session calls and collapses every failure into a plain result object; [`ui/modules/sidebar/components/CalendarExport.tsx`](../apps/web/src/ui/modules/sidebar/components/CalendarExport.tsx) uses scoped acquire/release to guarantee an object URL is revoked after the PDF download; and [`infrastructure/services/holidays/getHolidays.ts`](../apps/web/src/infrastructure/services/holidays/getHolidays.ts) wraps a synchronous computation that can throw. All of them are orchestration of fallible side effects, which is the same reason as on the server. But it does mean Effect is not confined to the server.

The browser Stripe client was on that list, and is not any more. [`infrastructure/clients/payments/stripe/client.ts`](../apps/web/src/infrastructure/clients/payments/stripe/client.ts) is now a lazy `loadStripe` singleton with no `effect` import at all: the confirm-and-classify chain it used to hold moved into the checkout adapter, which is the first entry above. Counting it again is the mistake this paragraph exists to stop, because a reader working from a stale list will either hunt for an entry that is not there or put Effect back into a module that was deliberately emptied.
